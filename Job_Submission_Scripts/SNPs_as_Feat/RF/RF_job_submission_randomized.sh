...
# Baseline models using randomized labels and Test.txt
...
python ${path1}/ML_regression.py -df ${data}/geno.csv -df2 ${data}/pheno_randomized.csv -y_name ${TRAITS[${SLURM_ARRAY_TASK_ID}]} -sep , -test ${data}/Test.txt -alg RF -n_jobs 12 -n 10 -cv_num 5 -save ${TRAITS[${SLURM_ARRAY_TASK_ID}]}_rf_base_rand -plots t
