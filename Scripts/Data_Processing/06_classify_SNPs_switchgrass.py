...
for inl in gff:
	if not inl.startswith('#'):
		...
		print("tem:", tem, "chr:", chr, "type:", type, "name:", name)
		if type == 'gene':
			G[name] = {}
		if type == 'mRNA':
			gene = '.'.join(name.split('.')[0:2])
			G[gene][name] = {}
			print("mRNA gene:", gene, "; G[gene][mRNA]:", G[gene][mRNA])
		if type == 'CDS':
			gene = '.'.join(name.split('.')[0:2])
			mRNA = '.'.join(name.split('.')[0:3])
			print("CDS gene:", gene, "; name:", name)
			print("G[gene][mRNA]:", gene[gene][mRNA])
			if 'CDS' not in G[gene][mRNA]:
				G[gene][mRNA]['CDS'] = []
			G[gene][mRNA]['CDS'].append(name)
			print("G[gene][mRNA]['CDS']:", G[gene][mRNA]['CDS'])
		if type == 'five_prime_UTR':
			gene = '.'.join(name.split('.')[0:2])
			mRNA = '.'.join(name.split('.')[0:3])
			print("5'UTR gene:", gene, "; mRNA:", mRNA)
			if 'five_prime_UTR' not in G[gene][mRNA]:
				G[gene][mRNA]['five_prime_UTR'] = []
			G[gene][mRNA]['five_prime_UTR'].append(name)
			print("G[gene][mRNA]['five_prime_UTR']:", G[gene][mRNA]['five_prime_UTR'])
