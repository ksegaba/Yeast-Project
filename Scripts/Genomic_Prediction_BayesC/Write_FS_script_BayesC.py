...
and BayesC using the FS files for each trait. 

Inputs:
- path1 = path to BL pipeline scripts e.g. /mnt/home/seguraab/Shiu_Lab/Project/Scripts/Genomic_Prediction_BayesC
- path2 = path to save FS files & RF output  e.g /mnt/scratch/seguraab/yeast_project/yeast_BayesC_results
- path3 = path to BL input files (genomic) e.g. /mnt/home/seguraab/Shiu_Lab/Project/Data/Peter_2018
- save = path to save job_submission files e.g. /mnt/home/seguraab/Shiu_Lab/Project/Job_Submission_Scripts/SNPs_as_Feat/BayesC
...
Command: 
conda activate
python Write_FS_script_BayesC.py -path1 /mnt/home/seguraab/Shiu_Lab/Project/Scripts/Genomic_Prediction_BayesC -path2 /mnt/scratch/seguraab/yeast_project/yeast_BayesC_results -path3 /mnt/home/seguraab/Shiu_Lab/Project/Data/Peter_2018 -save /mnt/home/seguraab/Shiu_Lab/Project/Job_Submission_Scripts/SNPs_as_Feat/BayesC
...
"""def make_FS_files(path1, path2, path3, save, trait, start, stop, step, name1):
    ...
    for i in range(start,stop+step,step):
        out1.write('python %s/Feature_Selection.py -df %s/geno_BL_${trait}.csv -alg randomforest -n %i -test /mnt/home/seguraab/Shiu_Lab/Project/Data/Peter_2018/Test.txt -y_name Y -sep , -type r -save feat_BayesC_${trait}_top -scores T\n'%(path1,path3,i))
...
def make_BayesC_files(path1, path2, path3, trait, name):
    # Write BayesC slurm job submission script
    out = open(name, 'w')
    out.write(f'#!/bin/sh --login \
                ...
                \n#SBATCH --mem=36G \
                \n#SBATCH --job-name BayesC_job_{trait} \
                \n#SBATCH --output=%x_%j \
                \ncd {path2} \
                \nmodule purge \
                \nmodule load GCC/8.3.0  OpenMPI/3.1.4  R/4.0.2\n')
    
    out.write('Rscript %s/BayesC_Model.R %s/geno.csv %s/pheno.csv all %s/Test.txt %s %s/CVFs.csv 5 10 BayesC_geno %s'%(path1,path3,path3,path3,trait,path3,path2))
    out.write('\nscontrol show job $SLURM_JOB_ID')
    out.close()
...
def main():
    ...
    for trait in traits:
        print(trait)
        # Write slurm scripts
        name = '%s/BayesC_job_submission_%s.slurm'%(save,trait)
        make_BayesC_files(path1, path2, path3, trait, name)
