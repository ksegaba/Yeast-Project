...
genes <- read.csv("Data/Peter_2018/ORFs_gene_map.tsv", sep="\t", header=T)
pwy2 <- left_join(pwy, genes, by=c("Accession.1"="gene")) # merge
with_pwy <- pwy2[pwy2$Pathways.of.gene!="",] # genes with pathway annotations
sum(is.na(with_pwy$qacc)) # check for no missing data
with_pwy <- with_pwy[which(!is.na(with_pwy$qacc)),]
...
get_genes <- function(f) {
    ...
    out <- right_join(with_pwy, df, by=c("qacc"="ORF")) # add gene & pwy information
...
pwy_enrichment <- function(f){
    ...
    # add gene information to top orf file
    top <- get_genes(f)

    # drop orfs without pathway annotations
    ...
    bg <- bg[c(2,5:8)] # keep only Accession.1, Product, Pathways.of.gene, qacc, and sacc
    ...
    ## Overrepresentation Analysis
    path <- paste("Scripts/Genomic_Prediction_RF/SHAP/ORFs/fs/PWY_ORA_", str_extract(f,
