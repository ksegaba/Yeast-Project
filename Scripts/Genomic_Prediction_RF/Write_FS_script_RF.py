...
python /mnt/home/seguraab/Shiu_Lab/Project/Scripts/Genomic_Prediction_RF/Write_FS_script_RF.py -path1 ${path1} -path2 ${path2} -path3 ${path3} -save ${save} -start 500 -stop 50000 -step 500 -runFS n -runRF y -submit y
python /mnt/home/seguraab/Shiu_Lab/Project/Scripts/Genomic_Prediction_RF/Write_FS_script_RF.py -path1 ${path1} -path2 ${path2} -path3 ${path3} -save ${save} -start 1 -stop 12 -base 2 -runExpFS y -submit y
...
def make_exp_FS_files(path1, path2, path3, save, trait, start, stop, base, name1):
    # Write feature selection slurm job submission script
    out1 = open(name1, 'w')
    out1.write(f'#!/bin/sh --login \
                \n#SBATCH --time=3:00:00 \
    ...
    for i in feats:
        out1.write('python %s/Feature_Selection.py -df %s/geno_rf_${trait}.csv -alg randomforest -n %i -test /mnt/home/seguraab/Shiu_Lab/Project/Data/Peter_2018/Test.txt -y_name Y -sep , -type r -save feat_exp_rf_${trait}_top -scores T\n'%(path1,path3,i))
        out1.write('python %s/ML_regression.py -df /mnt/scratch/seguraab/yeast_project/yeast_rf_results/geno_rf_${trait}.csv -sep , -feat /mnt/scratch/seguraab/yeast_project/yeast_rf_results/feat_exp_rf_${trait}_top_%i -test /mnt/home/seguraab/Shiu_Lab/Project/Data/Peter_2018/Test.txt -alg RF -n_jobs 12 -n 100 -cv_num 5 -save ${trait}_exp_rf_%i -plots t\n\n'%(path1,i,i))
    out1.write('\nscontrol show job $SLURM_JOB_ID')
    out1.close()

def make_RF_files(path1, path2, path3, save, trait, start, stop, step, name2):
...
def main():
    ...
    base = int(args.base) # base of exponential
    ...
    for trait in traits:
        ...
        name3 = '%s/FS_exp_files_for_RF_%s.slurm'%(save, trait)
        ...
        if (args.runExpFS == 'y'):
            make_exp_FS_files(path1, path2, path3, save, trait, start, stop, base, name3)
            if (args.submit == 'y'):
                os.system(f'sbatch {name3}') # to create feature selection files
        if (args.runRF == 'y'):
            make_RF_files(path1, path2, path3, save, trait, start, stop, step, name2)
            if (args.submit == 'y'):
