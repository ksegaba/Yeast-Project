...
- runFS = make FS files slurm scripts (y/n), default is y
...
def make_FS_files(path1, path2, path3, save, trait, start, stop, step, snp=False, orf=False, cno=False):
...
def make_exp_FS_files(path1, path2, path3, save, trait, start, stop, base, snp=False, orf=False, cno=False):
    ...
    out1.write(f'#!/bin/sh --login \
                ...
                \npath1={path1} \
                \npath3={path3}\n')
    feats = [base**i for i in range(start,stop)]
    for i in feats:
        if snp:
            out1.write('data=/mnt/home/seguraab/Shiu_Lab/Project/Data/Peter_2018/ \
                        \npython ${path1}/Feature_Selection.py -df ${path3}/geno_rf_${trait}.csv -alg randomforest -n %i -test ${data}/Test.txt -y_name Y -sep , -type r -save feat_exp_rf_${trait}_top -scores T\n \
                        \npython ${path1}/ML_regression.py -df ${path3}/geno_rf_${trait}.csv -sep , -feat ${path3}/feat_exp_rf_${trait}_top_%i -test ${data}/Test.txt -alg RF -n_jobs 12 -n 100 -cv_num 5 -save ${trait}_exp_rf_%i -plots t\n\n')
        if orf:
            out1.write('python ${path1}/Feature_Selection.py -df ${path3}/ORFs_pres_abs.csv -df2 ${path3}/pheno.csv -y_name ${trait} -alg randomforest -n %i -test ${path3}/Test.txt -sep , -type r -save feat_exp_rf_${trait}_top -scores T\n')
            out1.write('python ${path1}/ML_regression.py -df ${path3}/ORFs_pres_abs.csv -df2 ${path3}/pheno.csv -y_name ${trait} -sep , -feat ${path3}/feat_exp_rf_${trait}_top_%i -test ${path3}/Test.txt -alg RF -n_jobs 12 -n 100 -cv_num 5 -save ${trait}_exp_rf_%i -plots t\n\n')
        if cno:
            out1.write('python ${path1}/Feature_Selection.py -df ${path3}/ORFs_no_NA.csv -df2 ${path3}/pheno.csv -y_name ${trait} -alg randomforest -n %i -test ${path3}/Test.txt -sep , -type r -save feat_exp_rf_${trait}_top -scores T\n')
            out1.write('python ${path1}/ML_regression.py -df ${path3}/ORFs_no_NA.csv -df2 ${path3}/pheno.csv -y_name ${trait} -sep , -feat ${path3}/feat_exp_rf_${trait}_top_%i -test ${path3}/Test.txt -alg RF -n_jobs 12 -n 100 -cv_num 5 -save ${trait}_exp_rf_%i -plots t\n\n')
    out1.write('\nscontrol show job $SLURM_JOB_ID')
    out1.close()
    return name1
...
def make_RF_files(path1, path2, path3, save, trait, start, stop, step, batch, snp=False, orf=False, cno=False):
    ...
    if snp:
        out2.write('\n\tpython ${path1}/ML_regression.py -df ${path3}/geno_rf_${trait}.csv -sep , -feat ${path3}/feat_rf_${trait}_top_$i -test /mnt/home/seguraab/Shiu_Lab/Project/Data/Peter_2018/Test.txt -alg RF -n_jobs 12 -n 100 -cv_num 5 -save ${trait}_rf_$i -plots t')
    if orf:
        out2.write('\n\tpython ${path1}/ML_regression.py -df ${path3}/ORFs_pres_abs.csv -df2 ${path3}/pheno.csv -y_name ${trait} -sep , -feat ${path3}/feat_rf_${trait}_top_$i -test ${path3}/Test.txt -alg RF -n_jobs 12 -n 100 -cv_num 5 -save ${trait}_rf_$i -plots t')
    if cno:
        out2.write('\n\tpython ${path1}/ML_regression.py -df ${path3}/ORFs_no_NA.csv -df2 ${path3}/pheno.csv -y_name ${trait} -sep , -feat ${path3}/feat_rf_${trait}_top_$i -test ${path3}/Test.txt -alg RF -n_jobs 12 -n 100 -cv_num 5 -save ${trait}_rf_$i -plots t')
    out2.write('\ndone\n \
                \nscontrol show job $SLURM_JOB_ID')
    out2.close()
    return name2
...
def main():
    ...
    # Optional input
    req_group.add_argument('-step', help='step size (generate a FS file at each step)', default=500)
    req_group.add_argument('-batch', help='number of features each array task gets e.g. 5000', default=5000)
    req_group.add_argument('-base', help='base (generate a FS file from base ** start to base ** stop) e.g. 2', default=2)
    req_group.add_argument('-runFS', help='make FS files slurm scripts (y/n)', default='n')
    req_group.add_argument('-runExpFS', help='make FS files slurm scripts based on exponentially picking features, default is n',default='n')
    req_group.add_argument('-runRF', help='make RF slurm scripts (y/n)', default='n')
    req_group.add_argument('-snp', help='set to True if features are SNPs', default=False)
    req_group.add_argument('-orf', help='set to True if features are ORF presence/absence', default=False)
    req_group.add_argument('-cno', help='set to True if features are copy number', default=False)
    req_group.add_argument('-submit', help='submit slurm scripts (y/n)', default='n')
    # Help
    if len(sys.argv)==1:
        parser.print_help()
        sys.exit(0)
    ...
    for trait in traits:
        ...
        if (args.runFS == 'y'):
            name = make_FS_files(path1, path2, path3, save, trait, start, stop, step, snp=args.snp, orf=args.orf, cno=args.cno)
            if (args.submit == 'y'):
                os.system(f'sbatch {name}') # to create feature selection files
        if (args.runExpFS == 'y'):
            name = make_exp_FS_files(path1, path2, path3, save, trait, start, stop, base, snp=args.snp, orf=args.orf, cno=args.cno)
            if (args.submit == 'y'):
                os.system(f'sbatch {name}') # to create feature selection files
        if (args.runRF == 'y'):
            name = make_RF_files(path1, path2, path3, save, trait, start, stop, step, batch, snp=args.snp, orf=args.orf, cno=args.cno)
            if (args.submit == 'y'):
