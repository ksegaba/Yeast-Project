...
pcc_btwn_data.to_csv("Scripts/Data_Vis/Section_2/Table_S2_baseline_performance_correlations_between_data_types_CORRECTED.tsv", index=True, sep="\t")
...
pcc_btwn_alg.to_csv("Scripts/Data_Vis/Section_2/Table_S2_baseline_performance_correlations_between_algorithms_CORRECTED.tsv", index=True, sep="\t")
...
pcc_btwn_env.to_csv("Scripts/Data_Vis/Section_2/Table_S2_baseline_performance_correlations_between_envs_across_data_types_CORRECTED.tsv", index=True, sep="\t")
...
pcc_btwn_env.columns = columns
pcc_btwn_env.to_csv("Scripts/Data_Vis/Section_2/Table_S2_baseline_performance_correlations_between_envs_across_algorithms_CORRECTED.tsv", index=True, sep="\t")
...
pcc_btwn_data_fs.to_csv("Scripts/Data_Vis/Section_2/Table_S2_FS_performance_correlations_between_data_types.tsv", index=True, sep="\t")
...
pcc_btwn_alg_fs.to_csv("Scripts/Data_Vis/Section_2/Table_S2_FS_performance_correlations_between_algorithms.tsv", index=True, sep="\t")
...
pcc_btwn_env.columns = columns
pcc_btwn_env.to_csv("Scripts/Data_Vis/Section_2/Table_S2_FS_performance_correlations_between_envs_across_data_types.tsv", index=True, sep="\t")
...
pcc_btwn_env.columns = columns
pcc_btwn_env.to_csv("Scripts/Data_Vis/Section_2/Table_S2_FS_performance_correlations_between_envs_across_algorithms.tsv", index=True, sep="\t")
...
for alg in ["RF", "XGBoost", "rrBLUP", "Bayesian LASSO", "BayesC"]:
    for alg2 in ["RF", "XGBoost", "rrBLUP", "Bayesian LASSO", "BayesC"]:
        for data in ["SNP", "PAV", "CNV"]:
            if alg != alg2:
                dist1 = res_fs.loc[(res_fs.Alg==alg) & (res_fs.Data==data), ["new_cond", "r2_test"]].set_index("new_cond")
                dist2 = res_fs.loc[(res_fs.Alg==alg2) & (res_fs.Data==data), ["new_cond", "r2_test"]].set_index("new_cond")
                stat, pval = mannwhitneyu(dist1, dist2, alternative="greater")
                rf_greater.append([f"{alg} > {alg2}?", data, "FS", stat[0], pval[0]])
...
for alg in ["RF", "XGBoost", "rrBLUP", "Bayesian LASSO", "BayesC"]:
    for alg2 in ["RF", "XGBoost", "rrBLUP", "Bayesian LASSO", "BayesC"]:
        for data in ["PCs_sklearn", "SNP", "PAV", "CNV"]:
            if alg != alg2:
                dist1 = res_base.loc[(res_base.Alg==alg) & (res_base.Data==data), ["new_cond", "r2_test"]].set_index("new_cond")
                dist2 = res_base.loc[(res_base.Alg==alg2) & (res_base.Data==data), ["new_cond", "r2_test"]].set_index("new_cond")
                stat, pval = mannwhitneyu(dist1, dist2, alternative="greater")
                rf_greater.append([f"{alg} > {alg2}?", data, "Baseline", stat[0], pval[0]])
                del dist1, dist2, stat, pval

rf_greater = pd.DataFrame(rf_greater, columns=["Comparison", "Data", "Model Type", "U", "p-value"])

_, q_values, _, _ = multipletests(rf_greater["p-value"].values, alpha=0.05, method='fdr_bh')

rf_greater.insert(5, "q-value", q_values)
rf_greater.\
    to_csv("Scripts/Data_Vis/Section_2/Table_S2_RF_vs_other_algs_mannwhitneyu_greater_CORRECTED.tsv", index=False, sep="\t")
