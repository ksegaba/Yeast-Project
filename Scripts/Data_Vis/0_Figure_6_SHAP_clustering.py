...
def sort_clusters(fitness, dend):  # df_top, iso_col
    ...
    # df_top_tmp, iso_col_tmp, dend_sorted)
...
def plotTree(D_dendro, color_list, nrow, ncol, idx_ax_dend, save: bool, save_dir, name=""):
    ...
    if save == True:
        ...
        extent = ax[idx_ax_dend].get_window_extent(
        ).transformed(fig.dpi_scale_trans.inverted())
        # pad the area by 15% and 10% in x and y directions
        fig.savefig(f"{save_dir}/{name}.pdf",
                    bbox_inches=extent.expanded(1.1, 1.15))
...
def cluster_shap_indiv(f, target_envs, pheno, iso_colors, plot: bool,
    ...
    if env in target_envs:
        ...
        # store the mann-whitney U test results
        mwu_res = {"fitness": {}, "shap": {}}
        ...
        top_feat = df.abs().median(axis=1).sort_values(
            ascending=False)[:20].index
        # ; print("Before clustering:", df_top.head())
        df_top = df.loc[top_feat, :]
        # print(env, df_top.T.describe().T) # stats across isolates for each feature
        print(dtype, env, df_top.abs().median(axis=1).min(),
              df_top.abs().median(axis=1).max())
        #
        ...
        if save_out == True:
            ...
            Z_iso = hierarchy.linkage(
                df_top.T, method="ward", metric="euclidean")
            # og_leaves_list = hierarchy.leaves_list(Z_iso)
            Z_iso_ordered = hierarchy.optimal_leaf_ordering(
                Z_iso, df_top.T, metric="euclidean")
            optimal_leaves_list = hierarchy.leaves_list(Z_iso_ordered)
            dend = hierarchy.dendrogram(
                Z_iso_ordered, no_plot=True, color_threshold=dend_thrsh[env])
            print(
                f"Number of clusters: {len(np.unique(dend['leaves_color_list']))}")
            #
            # Now, cluster the features in the SHAP values dataframe
            Z_feat = hierarchy.linkage(
                df_top, method="ward", metric="euclidean")
            Z_feat_ordered = hierarchy.optimal_leaf_ordering(
                Z_feat, df_top, metric="euclidean")
            ...
            dend_iso = hierarchy.dendrogram(Z_iso_ordered, color_threshold=dend_thrsh[env],
                                            labels=df_top.iloc[:,
                                                               optimal_leaves_list].columns, ax=ax[0],
                                            leaf_font_size=2.5)
            '''
            ...
            # ; print("After clustering:", df_top.head())
            df_top = df_top.iloc[dend_feat["leaves"], dend_iso["leaves"]]
            if dtype == "snp":
                df_top.index = df_top.index.map(snp_id_map["actual_feature"])
            df_top.to_csv(
                f"{save_dir}/SHAP_clustered_{env}_top20_{dtype}_sorted_v5_actual_features.tsv", sep="\t")
            ...
            '''shap_med_per_cluster = df_top.T.abs().groupby(fitness_toplot.set_index("ID")[
                "Cluster_Color"]).median().median(axis=1)
            m, b, r_value, p_value, std_err = \
                linregress(shap_med_per_cluster.values,
                           cluster_medians.values.flatten())
            shap_v_fitness[env] = {'m': m, 'b': b,
                                   'r': r_value, 'pval': p_value, 'SE': std_err}
...
def cluster_shap_combined(snp_files, pav_files, cnv_files, target_envs, pheno, iso_colors, dend_thrsh, cbar_lim, save_dir, by="snp"):
    ...
    for env in target_envs:
        ...
        # top_snp, _, _ = cluster_shap_indiv(
        #     f_snp, target_envs, pheno, iso_colors, plot=False, save_out=True,
        #     dtype="snp", save_dir="/mnt/research/glbrc_group/shiulab/kenia/Shiu_Lab/Project/Scripts/Data_Vis/Section_6/shap_clusters/fs_top20",
        #     dend_thrsh=dend_thrsh_snp)
        # top_pav, _, _ = cluster_shap_indiv(
        #     f_pav, target_envs, pheno, iso_colors, plot=False, save_out=True,
        #     dtype="pav", save_dir="/mnt/research/glbrc_group/shiulab/kenia/Shiu_Lab/Project/Scripts/Data_Vis/Section_6/shap_clusters/fs_top20",
        #     dend_thrsh=dend_thrsh_pav)
        # top_cnv, _, _ = cluster_shap_indiv(
        #     f_cnv, target_envs, pheno, iso_colors, plot=False, save_out=True,
        #     dtype="cnv", save_dir="/mnt/research/glbrc_group/shiulab/kenia/Shiu_Lab/Project/Scripts/Data_Vis/Section_6/shap_clusters/fs_top20",
        #     dend_thrsh=dend_thrsh_cnv)
        if by == "snp":
...
if __name__ == "__main__":
    ...
    # fix ORF IDs to match map_orfs
    pav.columns = pav.apply(lambda x: re.sub("^X", "", x.name), axis=0)
    pav.columns = pav.apply(lambda x: re.sub("\.", "-", x.name), axis=0)
    # fix ORF IDs to match map_orfs
    cnv.columns = cnv.apply(lambda x: re.sub("^X", "", x.name), axis=0)
    ...
    for env in target_envs:
        for data_type in ["snp", "pav", "cnv"]:
            df = pd.read_csv(
                f"Scripts/Data_Vis/Section_6/shap_clusters/fs_top20/SHAP_clustered_{env}_top20_{data_type}_sorted_v4.tsv", sep="\t", index_col=0)
            df.T.corr(method="pearson").to_csv(
                f"Scripts/Data_Vis/Section_6/shap_clusters/fs_top20/SHAP_clustered_{env}_top20_{data_type}_sorted_v4_pcc_r_values.tsv", sep="\t")
            df.T.corr(method=lambda x, y: pearsonr(x, y)[1]).to_csv(
                f"Scripts/Data_Vis/Section_6/shap_clusters/fs_top20/SHAP_clustered_{env}_top20_{data_type}_sorted_v4_pcc_p_values.tsv", sep="\t")
            ...
            if data_type == "snp":  # Add benchmark gene information to the dataframe
                ...
                # fix ORF IDs to match map_orfs
                df.index = df.apply(lambda x: re.sub("^X", "", x.name), axis=1)
                df.index = df.apply(lambda x: re.sub(
                    "\.", "-", x.name), axis=1)
                df_info = pd.DataFrame(df.index).merge(
            ...
            for i in range(20):
                ...
                if data_type == "pav":
                    sns.scatterplot(x=pheno.loc[df.columns, env],
                                    y=pav.loc[df.columns,
                                              df.index[i]].astype(int),
                                    hue=df.iloc[i, :], ax=ax[i //
                                                             2][i % 2], hue_norm=norm,
                ...
                if data_type == "cnv":
                    sns.scatterplot(x=pheno.loc[df.columns, env],
                                    y=cnv.loc[df.columns,
                                              df.index[i]].astype(float),
                                    hue=df.iloc[i, :], ax=ax[i //
                                                             2][i % 2], hue_norm=norm,
                                    palette="RdBu_r", edgecolor="black", alpha=0.3, legend=False)
                ...
                sm.set_array([])
                plt.colorbar(sm, ax=ax[i//2][i % 2],
                             orientation="vertical", label="SHAP value")
            ...
            for i in range(20):
                for j in range(20):
                    if (i != j) & (i < j):
                        ...
                        if data_type == "pav":
                            size_column = pav.loc[df.columns,
                                                  df.index[i]].astype(int)
                            scatter = sns.scatterplot(x=df.iloc[i, :], y=df.iloc[j, :],
                                                      hue=pheno.loc[df.columns,
                                                                    env], ax=ax[i][j],
                        ...
                        if data_type == "cnv":
                            size_column = cnv.loc[df.columns,
                                                  df.index[i]].astype(int)
                            scatter = sns.scatterplot(x=df.iloc[i, :], y=df.iloc[j, :],
                                                      hue=pheno.loc[df.columns,
                                                                    env], ax=ax[i][j],
                            ...
                            ax[i][j].set_xlabel(f"{df.index[i]}")
                        if data_type == "snp":
                            size_column = snp.loc[df.columns,
                                                  df.index[i]].astype(int)
                            scatter = sns.scatterplot(x=df.iloc[i, :], y=df.iloc[j, :],
                                                      hue=pheno.loc[df.columns,
                                                                    env], ax=ax[i][j],
    ...
    res_df = pd.DataFrame(
        res, columns=["Environment", "Data_Type", "Feature", "R", "P-value"])
    res_df.to_csv(
        f"Scripts/Data_Vis/Section_6/shap_clusters/fs_top20/SHAP_clustered_FS_top20_indiv_feat_trends_pearsonr_v4.tsv")
    ...
    snp_files = [f"{path}/SNP/best_rf_fs/{f}" for f in files if re.search(
        r"SHAP_values_sorted_snp_[A-Za-z0-9_]+_[RFa-z0-9-_]+_training\.txt", f)]
    files = os.listdir(f"{path}/PAV/best_rf_fs")
    pav_files = [f"{path}/PAV/best_rf_fs/{f}" for f in files if re.search(
        r"SHAP_values_sorted_pav_[A-Za-z0-9_]+_[RFa-z0-9-_]+_training\.txt", f)]
    files = os.listdir(f"{path}/CNV/best_rf_fs")
    cnv_files = [f"{path}/CNV/best_rf_fs/{f}" for f in files if re.search(
        r"SHAP_values_sorted_cnv_[A-Za-z0-9_]+_[RFa-z0-9-_]+_training\.txt", f)]

    # Cluster SHAP values
    target_envs = ["YPDCAFEIN40", "YPDCAFEIN50",
                   "YPDSODIUMMETAARSENITE", "YPDBENOMYL500", "YPDCUSO410MM"]
    dend_thrsh_snp = {"YPDCAFEIN40": 0.03, "YPDCAFEIN50": 0.025,
                      "YPDSODIUMMETAARSENITE": 0.04, "YPDBENOMYL500": 0.03, "YPDCUSO410MM": 0.2}
    dend_thrsh_pav = {"YPDCAFEIN40": 0.045, "YPDCAFEIN50": 0.04,
                      "YPDSODIUMMETAARSENITE": 0.15, "YPDBENOMYL500": 0.06, "YPDCUSO410MM": 0.2}
    dend_thrsh_cnv = {"YPDCAFEIN40": 0.045, "YPDCAFEIN50": 0.05,
                      "YPDSODIUMMETAARSENITE": 0.3, "YPDBENOMYL500": 0.04, "YPDCUSO410MM": 1}
    ...
    for env in target_envs:
        for data_type in ["snp", "pav", "cnv"]:
            df = pd.read_csv(
                f"Scripts/Data_Vis/Section_6/shap_clusters/shap_int_best_rf_fs_top20/SHAP_clustered_{env}_top20_{data_type}_sorted_v2.tsv", sep="\t", index_col=0)
    ...
    res_df = pd.DataFrame(
        res, columns=["Environment", "Data_Type", "Feature", "R", "P-value"])
    res_df.to_csv(
        f"Scripts/Data_Vis/Section_6/shap_clusters/shap_int_best_rf_fs_top20/SHAP_clustered_FS_top20_indiv_feat_trends_pearsonr.tsv")
