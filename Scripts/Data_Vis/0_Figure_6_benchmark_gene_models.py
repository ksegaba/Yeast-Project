...
res_lit = res[res.Tag.str.contains("baseline_only_benchmark")]
res_nlit = res[res.Tag.str.contains("fs_only_important_non_bench")]
res_combined = res[res.Tag.str.contains("important_non_bench_genes_plus_bench")]

res_combined.insert(2, "Data", res.Tag.str.split("_", expand=True)[0]) # add data column
...
for i,data_type in enumerate(["snp", "pav", "cnv"]):
	...
	ax[0][i].errorbar(
		x=res_combined.loc[res_combined.Data==data_type].sort_values(by="Tag")["Y"],
		y=res_combined.loc[res_combined.Data==data_type].sort_values(by="Tag")["r2_test"],
		yerr=res_combined.loc[res_combined.Data==data_type].sort_values(by="Tag")["r2_test_sd"],
		fmt='o', color='black', capsize=5)
	ax[0][i].set_title(f"{data_type} RF models with important non-bench plus bench features")
	...
	ax[1][i].errorbar(
		...
		y=res_nlit.loc[res_nlit.Data==data_type].sort_values(by="Tag")["r2_test"],
		yerr=res_nlit.loc[res_nlit.Data==data_type].sort_values(by="Tag")["r2_test_sd"],
		fmt='o', color='black', capsize=5)
	ax[1][i].set_title(f"{data_type} RF FS important non-bench genes only")
	...
	ax[2][i].errorbar(
		...
		y=res_lit.loc[res_lit.Data==data_type].sort_values(by="Tag")["r2_test"],
		yerr=res_lit.loc[res_lit.Data==data_type].sort_values(by="Tag")["r2_test_sd"],
		fmt='o', color='black', capsize=5)
	ax[2][i].set_title(f"{data_type}: only bench genes from baseline RF")
	ax[2][i].set_xticklabels(res_lit[res_lit.Data==data_type].sort_values(by="Tag").Y,
		size=7, rotation=50, ha="right")


plt.tight_layout()
plt.savefig("Scripts/Data_Vis/Section_6/Figure_6_benchmark_gene_model_performances_not_RF_FS.pdf")
plt.close()
