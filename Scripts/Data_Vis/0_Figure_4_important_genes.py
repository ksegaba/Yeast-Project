...
for data_type in ["snp", "pav", "cnv"]:
    for imp_type in ["imp", "shap"]:
        imp = pd.read_csv(
            f"Scripts/Data_Vis/Section_4/RF_FS_{imp_type}_{data_type}.tsv", sep="\t", index_col=0)
        imp = imp[["gene"] + target_envs]  # keep only the target envs
        imp = imp.loc[~imp.index.isna()]

        imp = imp.loc[imp.select_dtypes("number").dropna(
            how="all").index]  # drop irrelevant rows
...
def combine_2level_dict_dfs(combined_opt):
    ...
    for outer_key, inner_dict in combined_opt.items():
        out_df[outer_key] = pd.concat(
            inner_dict.values(), axis=1, keys=inner_dict.keys())
...
combined_opt_df.to_csv(
    "Scripts/Data_Vis/Section_4/Figure_4_benchmark_genes_in_opt_models_data.tsv", sep="\t")

# Draw the heatmap
combined_opt_df.fillna(0, inplace=True)  # set NaN values to 0 importance
combined_opt_df_rank = combined_opt_df.rank(
    method="average", ascending=False)  # assign importance scores to rankings
...
plt.tight_layout()
plt.savefig(
    "Scripts/Data_Vis/Section_4/Figure_4_benchmark_genes_in_opt_models.pdf")
plt.close()
...
for data_type in ["snp", "pav", "cnv"]:
    for imp_type in ["imp", "shap"]:
        rank_per = pd.read_csv(
            f"Scripts/Data_Vis/Section_4/RF_FS_{imp_type}_{data_type}_rank_per.tsv", sep="\t", index_col=0)
        ...
        for env in target_envs:
            top_20.append(rank_per.sort_values(
                by=env, ascending=False)[env].iloc[:20])
...
combined_top_20_df["Benomyl"] = combined_top_20_df.index.isin(
    benomyl["Gene Systematic Name"].values).astype(int)
combined_top_20_df["Caffeine"] = combined_top_20_df.index.isin(
    caffeine["Gene Systematic Name"].values).astype(int)
combined_top_20_df["CuSO4"] = combined_top_20_df.index.isin(
    cu["Gene Systematic Name"].values).astype(int)
combined_top_20_df["Sodium_meta-arsenite"] = combined_top_20_df.index.isin(
    sma["Gene Systematic Name"].values).astype(int)
combined_top_20_df["Curated"] = combined_top_20_df.index.isin(
    curated["gene"].values).astype(int)

combined_top_20_df[['Benomyl', 'Caffeine', 'CuSO4',
                    'Sodium_meta-arsenite', 'Curated']].sum()
# Benomyl                      9
# Caffeine                    19
# CuSO4                        7
...
for env in target_envs:
    env_df = combined_top_20_df.loc[:, combined_top_20_df.columns.get_level_values(
        2) == env].dropna(how="all")
    env_df.sort_values(by=("gini", "snp", env), ascending=False, inplace=True)
    env_df = env_df.rank(ascending=False)

    env_df["Benomyl"] = env_df.index.isin(
        benomyl["Gene Systematic Name"].values).astype(int)
    env_df["Caffeine"] = env_df.index.isin(
        caffeine["Gene Systematic Name"].values).astype(int)
    env_df["CuSO4"] = env_df.index.isin(
        cu["Gene Systematic Name"].values).astype(int)
    env_df["Sodium_meta-arsenite"] = env_df.index.isin(
        sma["Gene Systematic Name"].values).astype(int)
    ...
    plt.savefig(
        f"Scripts/Data_Vis/Section_4/Figure_4_top_20_features_{env}_rank_percentiles.pdf")
    plt.close()

    # Save env_df
    env_df.to_csv(
        f"Scripts/Data_Vis/Section_4/Figure_4_top_20_features_{env}_rank_percentiles.tsv", sep="\t")
    del env_df
